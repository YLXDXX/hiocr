...
该项目是一个基于 Qt6 框架开发的 ~~跨平台~~ 光学字符识别（OCR）桌面工具，可识别文字、公式和表格，特别针对 Linux 桌面 Wayland 环境进行了优化，在 KDE 桌面下体验最好。
...
- 对于纯数学公式，配制不同的处理脚本
- 兼容网络在线 OpenAI 接口
- 支持静默识别（不展示主窗口，可以通过托盘状态、鼠标形状、顶层悬浮窗口图形展示三种方式进行是否识别完成提示，分三种，识别错误、识别中、识别完成）
